...
def encrypt_vigenere(plaintext: str, keyword: str) -> str:
    ...
    for let_ind, letter in enumerate(plaintext):
        if letter.isalpha():
            shift = ord(keyword[let_ind % len(keyword)]) - ord_islower_ind[True]
            ciphertext += chr((ord(letter) - ord_islower_ind[letter.islower()] + shift) % alphabet_length + ord_islower_ind
            [
            letter.islower()
            ] )
        else:
